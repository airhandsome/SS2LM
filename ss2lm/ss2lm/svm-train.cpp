...
void exit_with_help()
{
	printf(
		...
		"-v1 for ss2lm v1"
		"-v2 for ss2lm v2"
		"-si for ss2lm sigma   the ratio of inner margin and outer margin"
...
int main(int argc, char **argv)
{
	...
	//int argc = 19;
	//	char *argv[] = { "svdd-train.exe", "-s", "8", "-t", "2", "-c", "100", "-b", "1", "-vm", "90", "-v1", "0.003", "-v2", "0.005", "-si", "2", "train1.txt", "new-ss2lm-reature_model3.txt" };
...
void parse_command_line(int argc, char **argv, char *input_file_name, char *model_file_name)
{
	...
	for (i = 1; i<argc; i++)
	{
		...
		case 's':
			if (argv[i - 1][2] == 'i'){
				param.sigma = atoi(argv[i]);
				break;
			}
			param.svm_type = atof(argv[i]);
			break;
		case 't':
		...
		case 'v':
			...
			if (argv[i - 1][2] == '1'){
				param.v1 = atof(argv[i]);
				break;
			}
			if (argv[i - 1][2] == '2'){
				param.v2 = atof(argv[i]);
				break;
			}
			cross_validation = 1;
			nr_fold = atoi(argv[i]);
